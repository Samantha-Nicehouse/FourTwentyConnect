...
namespace dk.via.ftc.businesslayer.Data.Services
{
    public class StrainAPIService : IStrainAPIService
    {
        ...
        public async Task<List<StrainAPIObj>> GetAllStrainsAsync()
        {
            ...
            {
                string strainName = messageTrim.Substring(0, messageTrim.IndexOf(":{")).Trim('"');
                string strainNameOut = messageTrim.Substring(0, messageTrim.IndexOf(":{"));// Gets the name of strain
                string strainNameCut = messageTrim.Replace(strainNameOut + ":", ""); // removes name of strain
                string strainCut;
